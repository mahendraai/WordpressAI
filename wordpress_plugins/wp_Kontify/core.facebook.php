...
class WpAutomaticFacebook extends wp_automatic
{
    ...
    public $wp_automatic_fb_cuser = '';
    public $wp_automatic_fb_xs = '';
    ...
    public function __construct()
    {
        ...
        //set public vars
        $this->wp_automatic_fb_cuser = wp_automatic_trim(get_option('wp_automatic_fb_cuser', ''));
        $this->wp_automatic_fb_xs = wp_automatic_trim(get_option('wp_automatic_fb_xs', ''));
    ...
    public function fb_get_post($camp)
    {
        ...
        // Authorization info
        $wp_automatic_fb_cuser = $this->wp_automatic_fb_cuser;
        $wp_automatic_fb_xs = $this->wp_automatic_fb_xs;
        ...
        if ((wp_automatic_trim($cg_fb_page_id) != '')) {
            ...
            if (1) {
                ...
                if ($cg_fb_from != 'events') {
                    ...
                    if($cg_fb_source == 'profile' && count($items) == 0){
                        ...
                        if(isset($matches[1]) && count($matches[1]) > 0){
                            ...
                            //test importing a specific post by id
                            //https://www.facebook.com/100064250362544/posts/1021236616694674
                            //$post_ids= array('1021236616694674');
                ...
                foreach ($items as $item) {
                    ...
                    //get the post content by URL
                    $exec2 = $this->grab_single_post($single_post_to_load);
                    ...
                    if (stristr($redirec_url, '/videos/') || stristr($redirec_url, '/watch/?v=') || stristr($exec2,'"videoId":"')) {
                    ...
                    if(preg_match('{^https?://}', $item)){
                        ...
                        $txtContent = '';
                        //if not empty and does not contain []} then it is a valid match
                        if(isset($full_text_matches[1]) && wp_automatic_trim($full_text_matches[1]) != '' && !stristr($full_text_matches[1], '[]}') ){
                    ...
                    if (  (!stristr($item, '</p></span>') && wp_automatic_trim($txtContent) != '' && !$is_colored_post)  ) {
                        ...
                        if (isset($full_text_matches[1]) && wp_automatic_trim($full_text_matches[1]) != '') {
                            ...
                            if (isset($full_text_matches[1]) && wp_automatic_trim($full_text_matches[1]) != '') {
                                ...
                                //if not empty and does not contain []} then it is a valid match
                                if (wp_automatic_trim($full_text_matches[1]) != ''  && !stristr($full_text_matches[1], '[]}') ) {
                    ...
                    try {
                         $full_imgs_srcs = $this->get_full_sized_images($exec2);
                    } catch (Exception $e) {
                        echo '<br>Failed to get full sized images';
                    }
    ...
    /**
     * Grabs a single post from Facebook.
     *
     * @param array $single_post_to_load An array containing the details of the single post to load.
     * @return string The content of the single post.
     */
    function grab_single_post($single_post_to_load){
        ...
        curl_setopt($this->ch, CURLOPT_COOKIE, 'xs=' . $this->wp_automatic_fb_xs . ';c_user=' . $this->wp_automatic_fb_cuser . ';datr=QGgFYdhWgha-QOwFUUksCDTg');
        curl_setopt($this->ch, CURLOPT_HTTPHEADER, array('sec-fetch-site: none', 'sec-fetch-mode: navigate', 'sec-fetch-user: ?1', 'sec-fetch-dest: document'));
    ...
    /**
     * Retrieves full-sized images from the provided execution context.
     *
     * @param mixed $exec2 The execution context or data from which full-sized images are to be retrieved.
     * @return array An array of full-sized images.
     */
    function get_full_sized_images($exec2){
  
        $full_imgs_srcs = array();
        ...
        //"attachment":{"media":{"__typename":"Photo"... "uri":"https:\/\/scontent.fc
        //https://pastebin.com/k5wesjAn
        }elseif (stristr($exec2,'"attachment":{"media":{"__typename":"Photo"')){

            //match
            preg_match_all('!"attachment":{"media":{"__typename":"Photo".*?"uri":"(.*?)"!', $exec2, $photo_matches);
        ...
        //<meta property="og:image" content="
        }elseif( stristr($exec2,'<meta property="og:image" content="')){

            //match
            preg_match_all('!<meta property="og:image" content="(.*?)"!s', $exec2, $photo_matches);
            ...
            if(isset($photo_matches[1]) && is_array($photo_matches[1])){
                ...
                echo '<br>Image found using og:image ';
        ...
        if(count($full_imgs_srcs) == 0){
            throw new Exception('No full image found');
        }else{
            return $full_imgs_srcs;
        }
    ...
    //function check if type is a video
