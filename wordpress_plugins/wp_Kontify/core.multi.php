...
$wp_automatic_options = get_option ( 'wp_automatic_options' );
	...
	if (in_array ( 'OPT_MULTI_FIXED_LIST', $camp_opt ) && wp_automatic_trim($camp_general['cg_multi_posts_list'] != '' ) ) {
		...
		if ($cg_ml_lnk_method == 'css' || $cg_ml_lnk_method == 'regex') {
			...
			if (count ( $link_matches [1] ) == 0) {
				...
				echo '<br>No href found, trying to get links directly by http...';
			...
			//if links were not found, explode the content by new line and check if every part is a numeric id
			if (count ( $found_links ) == 0) {
				echo '<br>No links found, checking if extractions are numeric id links';
				$suggested_values = array_values( array_filter( array_map('trim', explode("\n" , $finalContent )  )     )  );

				//check if all are numeric
				$all_numeric = false;

				if(count($suggested_values) > 0){

					foreach ( $suggested_values as $suggested_value ) {
						if (is_numeric($suggested_value)) {
							$all_numeric = true;
						}else{
							$all_numeric = false;
							break;
				...
				if($all_numeric){
					...
					//add the domain before the numeric id
					$found_links = array();

					foreach ( $suggested_values as $suggested_value ) {
						$found_links[] = $site_domain_url . '/' . $suggested_value;
					}

					//titles
					$found_titles = $suggested_values;

					
				}else{
					echo '<br>Not all are numeric id links';
				}
