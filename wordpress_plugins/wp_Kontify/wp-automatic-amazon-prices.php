...
function wp_automatic_amazon_prices_update($using_api = true) {
	...
	//echo finding an amazon product to update
	echo '<br><br>Updating amazon prices for a single product';
	...
	if (count ( $rows ) > 0) {
		...
		if ($row->meta_value < $yesterday) {
			...
			echo '<br>Last update for post ' . $pid . ' was less than 24 hours ago pid:' . $row->post_id;
		}
	}else{
		echo '<br>No product found to update';
	}
}
